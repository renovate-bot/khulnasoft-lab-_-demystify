...
#run: build
#	$(BUILD_PATH) build -t demystify-example:latest -f .data/Dockerfile.example .
#
#run-large: build
#	$(BUILD_PATH) amir20/clashleaders:latest
#
#run-podman: build
#	podman build -t demystify-example:latest -f .data/Dockerfile.example .
#	$(BUILD_PATH) localhost/demystify-example:latest --engine podman
#
#run-podman-large: build
#	$(BUILD_PATH) docker.io/amir20/clashleaders:latest --engine podman
#
#run-ci: build
#	CI=true $(BUILD_PATH) demystify-example:latest --ci-config .data/.demystify-ci
#
#dev:
#	docker run -ti --rm -v $(PWD):/app -w /app -v demystify-pkg:/go/pkg/ golang:1.13 bash
#
#build: gofmt
...
generate-test-data:
	docker build -t demystify-test:latest -f .data/Dockerfile.test-image . && docker image save -o .data/test-docker-image.tar demystify-test:latest && echo 'Exported test data!'
	...
	docker run \
		...
		ubuntu:latest \
			/bin/bash -x -c "\
				...
				docker version && \
				apt install ./snapshot/demystify_*_linux_amd64.deb -y && \
				demystify --version && \
				demystify '${TEST_IMAGE}' --ci \
			"
		...
		fedora:latest \
			/bin/bash -x -c "\
				curl -L 'https://download.docker.com/linux/static/stable/x86_64/docker-${DOCKER_CLI_VERSION}.tgz' | \
					...
					mv docker /usr/local/bin/ &&\
				docker version && \
				dnf install ./snapshot/demystify_*_linux_amd64.rpm -y && \
				demystify --version && \
				demystify '${TEST_IMAGE}' --ci \
			"
...
ci-test-linux-run:
	ls -la $(SNAPSHOT_DIR)
	ls -la $(SNAPSHOT_DIR)/demystify_linux_amd64_v1
	chmod 755 $(SNAPSHOT_DIR)/demystify_linux_amd64_v1/demystify && \
	$(SNAPSHOT_DIR)/demystify_linux_amd64_v1/demystify '${TEST_IMAGE}'  --ci && \
    $(SNAPSHOT_DIR)/demystify_linux_amd64_v1/demystify --source docker-archive .data/test-kaniko-image.tar  --ci --ci-config .data/.demystify-ci
...
ci-test-mac-run:
	chmod 755 $(SNAPSHOT_DIR)/demystify_darwin_amd64_v1/demystify && \
	$(SNAPSHOT_DIR)/demystify_darwin_amd64_v1/demystify --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.demystify-ci

# we're not attempting to test docker, just our ability to run on these systems. This avoids setting up docker in CI.
.PHONY: ci-test-windows-run
ci-test-windows-run:
	demystify.exe --source docker-archive .data/test-docker-image.tar  --ci --ci-config .data/.demystify-ci
