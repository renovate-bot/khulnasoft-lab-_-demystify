...
import (
	...
	"github.com/khulnasoft-lab/demystify/demystify/filetree"
)
...
func initCli() {
	rootCmd.PersistentFlags().StringVar(&cfgFile, "config", "", "config file (default is $HOME/.demystify.yaml, ~/.config/demystify/*.yaml, or $XDG_CONFIG_HOME/demystify.yaml)")
	rootCmd.PersistentFlags().String("source", "docker", "The container engine to fetch the image from. Allowed values: "+strings.Join(demystify.ImageSources, ", "))
	...
	rootCmd.Flags().StringVar(&ciConfigFile, "ci-config", ".demystify-ci", "If CI=true in the environment, use the given yaml to drive validation rules.")
...
func initConfig() {
	...
	viper.SetDefault("log.path", "./demystify.log")
...
// and in $HOME/.config/demystify/ directory
// defaults to $HOME/.demystify.yaml
func getDefaultCfgFile() string {
	...
	return path.Join(home, ".demystify.yaml")
}

// findInPath returns first "*.yaml" file in path's subdirectory "demystify"
// if not found returns empty string
func findInPath(pathTo string) string {
	directory := path.Join(pathTo, "demystify")
